...
const PageHit = (clickHandler) => ({ hit }) => {
  ...
  const url = hit.url.replace("https://byteco.org", "")
  return (
    <div>
...
const indices = [
  { name: `prod-byteco-org`, title: `Pages`, hitComp: `PageHit` },
]

// Validate agaisnt basic requirements of an BTO address
    ...
    if (isValidAddress(state.query)) {
      return (
        <div>
          ...
          <p>
            <Translation id="search-bto-address" />{" "}
