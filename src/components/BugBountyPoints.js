...
const BugBountyPoints = () => {
  ...
  const data = useStaticQuery(graphql`
    query {
      dai: file(relativePath: { eq: "open/dai.png" }) {
        ...TokenLogo
      }
      ethLight: file(relativePath: { eq: "open/bto-black.png" }) {
        ...TokenLogo
      }
      ethDark: file(relativePath: { eq: "open/bto-orange.png" }) {
        ...TokenLogo
      }
    }
  `)
  ...
  return (
    <PointsExchange>
      <PointsExchangeLabel>
        <Translation id="page-open-bug-bounty-points-exchange" />{" "}
        <Tooltip content={tooltipContent}>
          <InfoIcon name="info" size="14" />
        </Tooltip>
      </PointsExchangeLabel>
      <PointsExchangeTitle>
        <Translation id="page-open-bug-bounty-points-point" />
      </PointsExchangeTitle>
      {state.hasError && (
        <ValueRow>
          <TokenValue>
            <Translation id="page-open-bug-bounty-points-error" />
          </TokenValue>
        </ValueRow>
      )}
      {isLoading && !state.hasError && (
        <ValueRow>
          <TokenValue>
            <Translation id="page-open-bug-bounty-points-loading" />
          </TokenValue>
        </ValueRow>
      )}
      {!isLoading && !state.hasError && (
        <ValueRow>
          <Row>
            ...
            <TokenValue>
              <Translation id="page-open-bug-bounty-points-usd" />
            </TokenValue>
          </Row>
          <Row>
      ...
      <p>
        <Translation id="page-open-bug-bounty-points-payout-desc" />
      </p>
      <TextNoMargin>
        <em>
          <Translation id="page-open-bug-bounty-points-rights-desc" />
        </em>
      </TextNoMargin>
    </PointsExchange>
  )
