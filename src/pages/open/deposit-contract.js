...
const DepositContractPage = ({ data, location }) => {
  ...
  const addressSources = [
    {
      title: "ConsenSys",
      link:
        "https://consensys.net/blog/news/open-phase-0-deposit-contract-address/",
      image: data.consensys.childImageSharp.fixed,
    },
    {
      title: "Byteco Foundation",
      link: "https://blog.byteco.org/2020/11/04/open-quick-update-no-19/",
  ...
  const textToSpeechText = state.isSpeechActive
    ? translateMessageId("page-open-deposit-contract-stop-reading", intl)
    : translateMessageId("page-open-deposit-contract-read-aloud", intl)
    ...
    <Page>
      <PageMetadata
        title={translateMessageId(
          "page-open-deposit-contract-meta-title",
          intl
        )}
        description={translateMessageId(
          "page-open-deposit-contract-meta-desc",
          intl
        )}
      />
      <LeftColumn>
        ...
        <Title>
          <Translation id="page-open-deposit-contract-title" />
        </Title>
        <Subtitle>
          <Translation id="page-open-deposit-contract-subtitle" />
        </Subtitle>
        <h2>
          <Translation id="page-open-deposit-contract-h2" />
        </h2>
        <p>
          <Translation id="page-open-deposit-contract-staking" />{" "}
          <Link to="/open/staking/">
            <Translation id="page-open-deposit-contract-staking-more-link" />
          </Link>
        </p>
        <StyledButton to="https://launchpad.byteco.org">
          <Translation id="page-open-deposit-contract-launchpad" />
        </StyledButton>
        <h2>
          <Translation id="page-open-deposit-contract-staking-check" />
        </h2>
        <p>
          <Translation id="page-open-deposit-contract-staking-check-desc" />
        </p>
      ...
      <RightColumn>
        <AddressCard>
          <CardTag>
            <Translation id="page-open-deposit-contract-address-check-btn" />
          </CardTag>
          <CardContainer>
            {!state.showAddress && (
              <>
                <Row>
                  <CardTitle>
                    <Translation id="page-open-deposit-contract-confirm-address" />
                  </CardTitle>
                </Row>
                <StyledCheckbox
                  size={1.5}
                ...
                >
                  <Translation id="page-open-deposit-contract-checkbox1" />
                </StyledCheckbox>
                <StyledCheckbox
                  size={1.5}
                ...
                >
                  <Translation id="page-open-deposit-contract-checkbox2" />
                </StyledCheckbox>
                <StyledCheckbox
                  size={1.5}
                ...
                >
                  <Translation id="page-open-deposit-contract-checkbox3" />
                </StyledCheckbox>
                <CopyButton
                ...
                >
                  ...
                  <Translation id="page-open-deposit-contract-reveal-address-btn" />
                </CopyButton>
              </>
            )}
            {state.showAddress && (
              <>
                <Row>
                  <TitleText>
                    <CardTitle>
                      <Translation id="page-open-deposit-contract-address" />
                    </CardTitle>
                    <Caption>
                      <Translation id="page-open-deposit-contract-address-caption" />
                    </Caption>
                  </TitleText>
                ...
                <Tooltip
                  content={translateMessageId(
                    "page-open-deposit-contract-warning",
                    intl
                  )}
                >
                ...
                <ButtonRow>
                      ...
                      <CopyButton>
                        {!isCopied ? (
                          <div>
                            ...
                            <Translation id="page-open-deposit-contract-copy" />
                          </div>
                        ) : (
                          <div>
                            <Emoji text=":white_check_mark:" size={1} />{" "}
                            <Translation id="page-open-deposit-contract-copied" />
                          </div>
                        )}
                      </CopyButton>
                    )}
                  ...
                  >
                    <Translation id="page-open-deposit-contract-etherscan" />
                  </Link>
                </ButtonRow>
              </>
            )}
            <InfoBanner isWarning={true} emoji=":warning:" mt={`2rem`}>
              <div>
                <Translation id="page-open-deposit-contract-warning-2" />{" "}
                <Link to="https://launchpad.byteco.org">
                  <Translation id="page-open-deposit-contract-launchpad-2" />
                </Link>
              </div>
            </InfoBanner>
          </CardContainer>
...
export const query = graphql`
  query {
    ...
    ef: file(relativePath: { eq: "open-staking/ef-blog-logo.png" }) {
      ...sourceImage
    }
